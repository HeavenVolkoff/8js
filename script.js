...
function emulator($, Chip8) {
	...
	romInput.change(function () {
		...
		fileReader.onloadend = function () {
			...
			if (!chip8) {
				chip8 = new Chip8(fileReader.result, {
					height: canvas.height,
					width: canvas.width
				}, {duration: audio.duration});

				chip8.on('playAudio', function () {
					audio.play();
				});
				chip8.on('stopAudio', function () {
					audio.pause();
					audio.currentTime = 0;
				});
				chip8.on('clearScreen', function (data) {
					context.clearRect(0, 0, canvas.width, canvas.height);
				});
				chip8.on('drawSprite', function (data) {
					context.fillStyle = data.color;
					context.fillRect(data.x, data.y, data.scale, data.scale);
				});

				$(document).keyup(function (event) {
					chip8.input.keyUp(event.keyCode | event.which);
				});

				$(document).keydown(function (event) {
					chip8.input.keyDown(event.keyCode | event.which);
				});
